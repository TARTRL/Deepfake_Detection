...
__all__ = ['EfficientNet', 'EfficientDeepfakeNet']
...
default_cfgs = {
    ...
    'efficientnet_b7_deepfake': _cfg(
        url='', input_size=(3, 450, 800), pool_size=(19, 19), crop_pct=0.949),
    'efficientnet_deepfake_v3': _cfg(
        url='', input_size=(12, 600, 600), pool_size=(19, 19), crop_pct=0.949),
    'efficientnet_deepfake_v4': _cfg(
        url='', input_size=(12, 600, 600), pool_size=(19, 19), crop_pct=0.949),
...
class EfficientDeepfakeNet(nn.Module):
    """ (Generic) EfficientNet
    ...
    def __init__(self, block_args, num_classes=1000, num_features=1280, in_chans=3, stem_size=32,
                 channel_multiplier=1.0, channel_divisor=8, channel_min=None,
                 output_stride=32, pad_type='', act_layer=nn.ReLU, drop_rate=0., drop_path_rate=0.,
                 se_kwargs=None, norm_layer=nn.GroupNorm, norm_kwargs=None, global_pool='avg'):
        super(EfficientDeepfakeNet, self).__init__()
        # print('use EfficientDeepfakeNet')
        norm_kwargs = norm_kwargs or {}
        ...
        if self.use_norm:
            num_groups = 8
            self.gn1 = norm_layer(num_groups=num_groups, num_channels=stem_size, **norm_kwargs)
        # self.bn1 = norm_layer(stem_size, track_running_stats=False)
        ...
        # Middle stages (IR/ER/DS Blocks)
        builder = EfficientDeepfakeNetBuilder(
            channel_multiplier, channel_divisor, channel_min, output_stride, pad_type, act_layer, se_kwargs,
            norm_layer, norm_kwargs, drop_path_rate, verbose=_DEBUG)
        self.blocks = nn.Sequential(*builder(self._in_chs, block_args))
        self.feature_info = builder.features
        ...
        # Head + Pooling
        self.conv_head = create_conv2d(self._in_chs, self.num_features, 1, padding=pad_type)
        if self.use_norm: self.gn2 = norm_layer(num_groups=num_groups, num_channels=self.num_features, **norm_kwargs)
        # self.bn2 = norm_layer(self.num_features, momentum=0)
        # self.bn2 = norm_layer(self.num_features, affine=True,track_running_stats=False)
        self.act2 = act_layer(inplace=True)
        self.global_pool = SelectAdaptivePool2d(pool_type=global_pool)
    ...
    def forward_features(self, x):
        x = self.conv_stem(x)
        if self.use_norm: x = self.gn1(x)
        x = self.act1(x)
        # print(x.shape)
        x = self.blocks(x)
        # print(x.shape)
        x = self.conv_head(x)
        if self.use_norm: x = self.gn2(x)
        x = self.act2(x)
        return x
...
def _create_deepfake_model(model_kwargs, default_cfg, pretrained=False):
    if model_kwargs.pop('features_only', False):
        load_strict = False
        model_kwargs.pop('num_classes', 0)
        model_kwargs.pop('num_features', 0)
        ...
        load_strict = True
        # model_class = EfficientDeepfakeNet
        model_class = EfficientNet
...
def _gen_efficientnet_deepfake(variant, channel_multiplier=1.0, depth_multiplier=1.0, pretrained=False, **kwargs):
    ...
    model = _create_deepfake_model(model_kwargs, default_cfgs[variant], pretrained)
    return model
...
def efficientnet_b7_deepfake(pretrained=False, **kwargs):
    ...
    # NOTE for train, drop_rate should be 0.5, drop_path_rate should be 0.2
    model = _gen_efficientnet(
        'efficientnet_b7_deepfake', channel_multiplier=2.0, depth_multiplier=3.1, pretrained=pretrained, **kwargs)
    return model
...
def efficientnet_deepfake_v3(pretrained=False, **kwargs):
    ...
    model = _gen_efficientnet_deepfake(
        'efficientnet_deepfake_v3', channel_multiplier=2.0, depth_multiplier=3.1, pretrained=pretrained, **kwargs)
    return model
...
def efficientnet_deepfake_v4(pretrained=False, **kwargs):
    ...
    # NOTE for train, drop_rate should be 0.5, drop_path_rate should be 0.2
    model = _gen_efficientnet_deepfake(
        'efficientnet_deepfake_v4', channel_multiplier=2.0, depth_multiplier=3.1, pretrained=pretrained, **kwargs)
    return model
